...
<template>
  <div class="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#B6F500] selection:text-black">
    ...
    <!-- 1. LOGIN PAGE -->
    <div v-if="currentPage === 'login'" class="min-h-screen w-full flex items-center justify-center relative overflow-hidden">
      ...
      <div class="w-full max-w-md p-10 backdrop-blur-2xl bg-white/5 border border-white/10 rounded-[40px] shadow-2xl relative z-10">
        <div class="flex flex-col items-center mb-10">
          ...
          <h1 class="text-3xl font-black text-white tracking-tight">RMA <span class="text-[#B6F500]">SYSTEM</span></h1>
          <p class="text-white/40 mt-2 font-medium">Portal Operasional Internal</p>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="flex items-center justify-between text-xs text-white/40">
            <label class="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
              <input type="checkbox" class="rounded border-white/10 bg-white/5 accent-[#B6F500]" > Ingat Sesi
            </label>
            <a href="#" class="hover:text-[#B6F500] transition-colors">Lupa Password?</a>
          </div>
          <button 
        ...
        <p class="mt-10 text-center text-[10px] text-white/20 uppercase tracking-[0.2em]">Build version 4.0.1-stable</p>
      </div>
    </div>
    ...
    <div v-else class="flex min-h-screen">
      ...
      <main class="flex-1 flex flex-col">
        ...
        <div class="flex-1 p-12 overflow-y-auto">
          ...
          <div v-if="currentPage === 'dashboard'" class="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
            <div class="flex justify-between items-end">
              <div>
                <h2 class="text-5xl font-black tracking-tighter uppercase italic">System <span class="text-[#B6F500]">Overview</span></h2>
                <p class="text-white/40 mt-2 font-medium">Monitoring performa operasional dan klaim vendor.</p>
              </div>
...
v-for="(kpi, i) in [
              ...
              ]" :key="i" class="p-8 backdrop-blur-xl bg-white/5 border border-white/10 rounded-4xl group hover:bg-white/10 transition-all cursor-pointer">
                <div class="flex justify-between items-start mb-6">
                  <div class="p-4 rounded-2xl bg-white/5 group-hover:bg-[#B6F500] group-hover:text-black transition-all">
                    <component :is="kpi.icon" :size="24" :class="i === 0 ? 'text-[#B6F500]' : 'text-white/40'" class="group-hover:text-inherit" />
                  </div>
                  <span class="text-[10px] font-black px-2.5 py-1 rounded-lg bg-[#B6F500]/10 text-[#B6F500] uppercase">{{ kpi.trend }}</span>
                </div>
                <p class="text-xs font-bold text-white/30 uppercase tracking-[0.2em] mb-1">{{ kpi.label }}</p>
                <h4 class="text-4xl font-black tracking-tight">{{ kpi.value }}</h4>
              </div>
            </div>
            ...
            <div class="grid grid-cols-12 gap-8">
              <div class="col-span-8 backdrop-blur-xl bg-white/5 border border-white/10 rounded-[40px] p-10">
                <div class="flex justify-between items-center mb-12">
                  <h3 class="text-xl font-black italic tracking-tight">KLAIM AKTIVITAS MINGGUAN</h3>
                  <div class="flex gap-2">
                    <div v-for="tag in ['Daily', 'Weekly']" :key="tag" :class="['px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest cursor-pointer transition-all', tag === 'Weekly' ? 'bg-[#B6F500] text-black' : 'bg-white/5 text-white/30 hover:text-white']">
                      {{ tag }}
                    </div>
                  </div>
                </div>
                ...
                <div class="h-64 flex items-end justify-between gap-6 px-4">
                  <div v-for="(h, idx) in [40, 65, 30, 95, 75, 50, 85]" :key="idx" class="flex-1 flex flex-col items-center group">
                    <div 
                      ...
                      :style="{ height: `${h}%` }"
                    >
                      <div v-if="idx === 3" class="absolute -top-10 left-1/2 -translate-x-1/2 bg-white text-black text-[10px] font-black px-2 py-1 rounded shadow-xl whitespace-nowrap">90 CLAIMS</div>
                    </div>
              ...
              <div class="col-span-4 space-y-6">
                <div class="bg-linear-to-br from-[#B6F500] to-[#8ac500] p-8 rounded-[40px] text-black flex flex-col justify-between h-56 relative overflow-hidden group cursor-pointer shadow-xl shadow-[#B6F500]/10">
                  <div class="relative z-10">
                    ...
                    <h4 class="text-2xl font-black leading-tight mt-2 italic">MTC Distribution butuh tindakan segera.</h4>
                  </div>
                  ...
                  <Package :size="140" class="absolute -bottom-8 -right-8 opacity-20 rotate-12 group-hover:rotate-0 transition-transform duration-700" />
                </div>

                <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-[40px] p-8">
                  <h3 class="font-black text-sm uppercase tracking-widest text-[#B6F500] mb-6 flex items-center gap-2"><TrendingUp :size="16"/> Top Performance</h3>
                  <div class="space-y-6">
                    <div
v-for="(v, i) in [
                      ...
                      <div class="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                        <div class="h-full rounded-full" :style="{ width: v.p, backgroundColor: v.color }"/>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-[40px] overflow-hidden">
              <div class="p-8 border-b border-white/5 flex justify-between items-center bg-white/2">
                <h3 class="text-xl font-black italic tracking-tight uppercase">Recent <span class="text-white/30">Activity</span></h3>
                <button class="text-xs font-black text-[#B6F500] uppercase tracking-widest hover:underline">View All Records</button>
              </div>
              <table class="w-full text-left">
                <thead class="bg-white/5">
                  <tr>
                    <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">ID Claim</th>
                    <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Agent</th>
                    <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Product Model</th>
                    <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Status</th>
                    <th class="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Action</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-white/5">
                  <tr v-for="(row, i) in claimsData" :key="i" class="hover:bg-white/5 transition-colors group">
                    <td class="px-8 py-5 font-black text-[#B6F500] italic">{{ row.id }}</td>
                    <td class="px-8 py-5">
                    ...
                    <td class="px-8 py-5 text-sm font-medium text-white/60">{{ row.prod }}</td>
                    <td class="px-8 py-5">
          ...
          <!-- PAGE: CS PORTAL -->
          <div v-if="currentPage === 'cs'" class="space-y-12 animate-in fade-in slide-in-from-bottom-5 duration-700">
            <!-- Hero Search -->
            <section class="relative rounded-[50px] p-20 overflow-hidden border border-[#B6F500]/20 bg-linear-to-br from-[#B6F500]/5 via-[#0a0a0a] to-[#0a0a0a]">
              ...
              <div class="max-w-3xl relative z-10">
                ...
                <p class="text-lg text-white/40 font-medium mb-12 leading-relaxed">Gunakan kode notifikasi POS untuk pengambilan data inventaris otomatis dan sinkronisasi status vendor.</p>
                ...
                <div class="mt-6 flex items-center gap-3 text-white/20">
                   <AlertCircle :size="14" />
                   <p class="text-[10px] font-bold uppercase tracking-widest italic">Hubungi Admin jika serial number tidak terdaftar di database pusat.</p>
                </div>
              </div>
            </section>
