...
import { claim, vendor, defectMaster, vendorClaimItem, productModel } from '#server/database/schema'
import { eq, and, sql, count, gte, lte, avg } from 'drizzle-orm'
...
export const reportService = {
  async getDashboardKpi(filter: ReportFilter): Promise<ExecutiveKpi> {
    ...
    // Average review lead time (SUBMITTED → APPROVED)
    // Using claim_history to find the time between SUBMIT and APPROVE actions
  ...
  async getBranchPerformance(filter: ReportFilter): Promise<BranchPerformanceRow[]> {
    ...
    // Compute avg lead time per branch using claim_history
  ...
  async getVendorPerformance(filter: ReportFilter): Promise<VendorPerformanceRow[]> {
    ...
    // Get vendor decision stats from vendor_claim_item
    ...
    for (const d of decisionRows) {
      decisionMap[d.vendorId] = {
        accepted: d.acceptedItems,
        rejected: d.rejectedItems,
        compensation: Number(d.totalCompensation)
      }
    }

    return claimRows.map((r) => {
      const decisions = decisionMap[r.vendorId] ?? { accepted: 0, rejected: 0, compensation: 0 }
  ...
  async getAgingAnalysis(filter: ReportFilter): Promise<AgingBucket[]> {
    ...
    // Only consider non-terminal claims (not APPROVED, not ARCHIVED)
