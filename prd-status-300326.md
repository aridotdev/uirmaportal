...
| CS-002 | `/cs/claims` | List klaim milik CS dengan kolom minimum, filter, search, pagination | Parsial | Perlu perbaikan | List, search, filter, pagination sudah ada | Tabel belum memuat seluruh kolom operasional minimum seperti notification code, vendor, defect, dan last update, serta filter/search belum cukup kaya untuk mendukung pencarian kerja harian CS | `app/pages/cs/claims/index.vue` | High |
...
| ID | Temuan | Status PRD | Checklist | File | Catatan | Prioritas |
| --- | --- | --- | --- | --- | --- | --- |
| CS-D-004 | List claim CS belum memiliki kolom minimum sesuai spec | Belum sesuai | Perlu perbaikan | `app/pages/cs/claims/index.vue` | PRD meminta claim number, notification code, model, vendor, defect, last update, dan status, sementara tabel saat ini belum menampilkan semua data itu secara eksplisit | High |
| CS-D-005 | Search/filter pada list claim CS masih terbatas | Parsial | Perlu perbaikan | `app/pages/cs/claims/index.vue` | Pencarian dan filter belum cukup untuk membantu CS menyaring klaim berdasarkan kombinasi data penting yang sering dipakai | Medium |
